...
def test_selection_respecte_le_seuil(client, creer_lot):
    creer_lot("Saumon", dlc=_dlc(0))
    creer_lot("Yaourt", dlc=_dlc(1))
    creer_lot("Carotte", dlc=_dlc(5))

    with connexion() as db:
        urgents = produits_urgents(db, seuil=1, aujourdhui=AUJOURDHUI)

    assert [u["nom"] for u in urgents] == ["Saumon", "Yaourt"]


def test_selection_signale_les_perimes(client, creer_lot):
...
def test_selection_ignore_clotures_et_sans_dlc(client, creer_lot):
    ...
    with connexion() as db:
        assert produits_urgents(db, seuil=1, aujourdhui=AUJOURDHUI) == []


def test_selection_regroupe_par_nom(client, creer_lot):
...
def test_rien_a_signaler_quand_le_frigo_est_sain(client, creer_lot, push_simule):
    ...
    assert executer()["statut"] == "rien_a_signaler"
...
def test_envoi_puis_garde_fou_du_jour(client, creer_lot, abonner, push_simule):
    ...
    assert executer()["statut"] == "deja_envoye"
    assert len(push_simule.envois) == 1

    # --force outrepasse le marqueur.
    assert executer(force=True)["statut"] == "envoye"
    assert len(push_simule.envois) == 2


def test_simulation_n_envoie_rien_et_ne_pose_pas_le_marqueur(
    client, creer_lot, abonner, push_simule
):
    abonner()
    creer_lot("Saumon", dlc=date.today().isoformat())

    resultat = executer(simuler=True)
    ...
    with connexion() as db:
        assert db.execute(
            "SELECT 1 FROM settings WHERE key = ?;", (CLE_DERNIER_ENVOI,)
        ).fetchone() is None


def test_marqueur_non_pose_si_aucun_envoi_n_aboutit(client, creer_lot, abonner, push_simule):
    """Sans abonne joignable, la journee doit rester a retenter."""
    endpoint = abonner()
    push_simule.refus[endpoint] = 500
    creer_lot("Saumon", dlc=date.today().isoformat())

    assert executer()["envoyes"] == 0
