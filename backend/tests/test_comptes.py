...
def test_le_premier_compte_adopte_le_frigo_d_avant_les_comptes(anonyme):
    """Une base anterieure aux comptes garde des lignes sans proprietaire."""
    with connexion() as db:
        db.execute(
            """
            INSERT INTO inventaire_frigo (utilisateur_id, nom, lot_id, date_achat)
            VALUES (NULL, 'Saumon historique', 'lot_ancien', '2026-09-01');
            """
        )

    anonyme.post(
        "/api/auth/inscription", json={"identifiant": "florian", "mot_de_passe": "motdepasse"}
    )

    assert [p["nom"] for p in anonyme.get("/api/produits").json()] == ["Saumon historique"]


def test_le_deuxieme_compte_n_adopte_rien(client, anonyme):
    """Seul le premier compte herite : les suivants demarrent a vide."""
    with connexion() as db:
        db.execute(
            """
            INSERT INTO inventaire_frigo (utilisateur_id, nom, lot_id, date_achat)
            VALUES (NULL, 'Orphelin', 'lot_orphelin', '2026-09-01');
            """
        )
    ...
    assert anonyme.get("/api/produits").json() == []
