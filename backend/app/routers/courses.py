...
def _charger(db: sqlite3.Connection, course_id: int) -> Course:
    ligne = db.execute("SELECT * FROM courses WHERE id = ?;", (course_id,)).fetchone()
    if ligne is None:
...
def lister_courses(db: sqlite3.Connection = Depends(get_db)) -> list[Course]:
...
def creer_course(corps: CourseACreer, db: sqlite3.Connection = Depends(get_db)) -> Course:
    nom = corps.nom.strip()
    if not nom:
        raise HTTPException(status.HTTP_400_BAD_REQUEST, "Le nom ne peut pas être vide.")

    curseur = db.execute(
        "INSERT INTO courses (nom, note, statut, date_creation) VALUES (?, ?, 'a_acheter', ?);",
        (nom, _nettoyer_note(corps.note), date.today().isoformat()),
    )
    return _charger(db, curseur.lastrowid)


@router.delete("/achetes", status_code=status.HTTP_204_NO_CONTENT)
def vider_achetes(db: sqlite3.Connection = Depends(get_db)) -> Response:
    """Retire de la liste tout ce qui a deja ete achete."""
    db.execute("DELETE FROM courses WHERE statut = 'achete';")
    return Response(status_code=status.HTTP_204_NO_CONTENT)


@router.patch("/{course_id}", response_model=Course)
def modifier_course(
    course_id: int, corps: CoursePatch, db: sqlite3.Connection = Depends(get_db)
) -> Course:
    _charger(db, course_id)
    ...
    if champs:
        ...
        db.execute(
            f"UPDATE courses SET {assignations} WHERE id = ?;", [*champs.values(), course_id]
        )
    return _charger(db, course_id)


@router.delete("/{course_id}", status_code=status.HTTP_204_NO_CONTENT)
def supprimer_course(course_id: int, db: sqlite3.Connection = Depends(get_db)) -> Response:
    curseur = db.execute("DELETE FROM courses WHERE id = ?;", (course_id,))
...
def marquer_achete(course_id: int, db: sqlite3.Connection = Depends(get_db)) -> Course:
    course = _charger(db, course_id)
...
def annuler_achat(course_id: int, db: sqlite3.Connection = Depends(get_db)) -> Course:
